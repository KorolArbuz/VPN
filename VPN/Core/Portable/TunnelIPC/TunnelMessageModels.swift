...
//  VPN
//
//  Versioned app <-> PacketTunnelExtension IPC contracts. The wire schema is
//  read-only and deliberately carries no credentials, raw URIs, or full backend
//  configuration.
//
...
nonisolated enum TunnelMessageKind: Equatable, Codable, Sendable {
    ...
    init(rawValue: String) {
        ...
        case "getHealthSnapshot":
            self = .getHealthSnapshot
        case "getRecentEvents":
            self = .getRecentEvents
        case "verifyKeychainSentinel":
            self = .verifyKeychainSentinel
        case "validateRuntimeConfiguration":
            self = .validateRuntimeConfiguration
        case "validateXrayConfiguration":
            self = .validateXrayConfiguration
        case "runXrayLifecycleSmokeTest":
            self = .runXrayLifecycleSmokeTest
        case "runXrayRemoteEgressProbe":
            self = .runXrayRemoteEgressProbe
        case "probeActiveXrayEgress":
            self = .probeActiveXrayEgress
        case "runUDPControlProbe":
            self = .runUDPControlProbe
        case "runLibXrayPingProbe":
            self = .runLibXrayPingProbe
        case "getDiagnosticProviderMode":
            self = .getDiagnosticProviderMode
        default:
...
nonisolated struct TunnelTrafficCounters: Codable, Equatable, Sendable {
    ...
    static let unavailable = TunnelTrafficCounters(
        bytesUploaded: nil,
        bytesDownloaded: nil,
        packetsUploaded: nil,
        packetsDownloaded: nil
    )
}
...
nonisolated struct TunnelTelemetrySnapshotRead: Equatable, Sendable {
    var stored: TunnelStoredTelemetrySnapshot
    var isStale: Bool
}
