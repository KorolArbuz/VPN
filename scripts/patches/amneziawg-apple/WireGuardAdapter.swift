...
/// Secret-free synchronous checkpoints for diagnosing failures before the
/// adapter initializer can return to the packet-tunnel provider.
public enum WireGuardAdapterInitializationCheckpoint: String, CaseIterable, Sendable {
...
private enum LoggerInstallationPolicy {
    case enabled
...
public class WireGuardAdapter {
    ...
    public typealias InitializationObserver = (WireGuardAdapterInitializationCheckpoint) -> Void

    private typealias GoLoggerCallback = @convention(c) (
    ...
    /// Synchronous observer for the constructor and first native calls.
    private let initializationObserver: InitializationObserver?

    /// Release builds can only use `.enabled`; the bypass case is compiled
    /// solely into DEBUG as a temporary diagnostic control.
    private let loggerInstallationPolicy: LoggerInstallationPolicy
    private var loggerInstalled = false

    /// C-ABI callback with static lifetime. It captures no Swift state; the
    /// adapter is recovered only from the context pointer supplied to Go.
    private static let goLoggerCallback: GoLoggerCallback = { context, logLevel, message in
        guard let context, let message else { return }
        let unretainedSelf = Unmanaged<WireGuardAdapter>.fromOpaque(context).takeUnretainedValue()
    ...
    /// - Parameter initializationObserver: a synchronous, secret-free diagnostic observer.
    public init(
        with packetTunnelProvider: NEPacketTunnelProvider,
        logHandler: @escaping LogHandler,
        initializationObserver: InitializationObserver? = nil
    ) {
        initializationObserver?(.wireGuardAdapterInitEntered)
        self.packetTunnelProvider = packetTunnelProvider
        initializationObserver?(.wireGuardAdapterProviderReferenceStored)
        self.logHandler = logHandler
        initializationObserver?(.wireGuardAdapterSwiftLogHandlerStored)
        self.initializationObserver = initializationObserver
        self.loggerInstallationPolicy = .enabled

        initializationObserver?(.wireGuardAdapterLoggerInstallationDeferred)
        initializationObserver?(.wireGuardAdapterInitCompleted)
    }

    #if DEBUG
    /// Diagnostic-only initializer used to prove whether the first Go call is
    /// the failing boundary. This overload does not exist in Release builds.
    public init(
        diagnosticWith packetTunnelProvider: NEPacketTunnelProvider,
        logHandler: @escaping LogHandler,
        initializationObserver: InitializationObserver? = nil,
        bypassLoggerInstallation: Bool
    ) {
        initializationObserver?(.wireGuardAdapterInitEntered)
        self.packetTunnelProvider = packetTunnelProvider
        initializationObserver?(.wireGuardAdapterProviderReferenceStored)
        ...
        initializationObserver?(.wireGuardAdapterSwiftLogHandlerStored)
        self.initializationObserver = initializationObserver
        self.loggerInstallationPolicy = bypassLoggerInstallation ? .diagnosticBypass : .enabled
    ...
    deinit {
        // Remove the logger only if registration completed. Calling into Go
        // during teardown before runtime initialization recreates the same
        // eager-call failure that this adapter deliberately avoids.
        if loggerInstalled {
            wgSetLogger(nil, nil)
        }
    ...
    /// Setup the WireGuard log handler only after the first successful Go
    /// backend call has proven that the embedded runtime is initialized.
    private func setupLogHandler() {
        guard loggerInstalled == false else { return }
        initializationObserver?(.wireGuardAdapterSetupLogHandlerEntered)
        initializationObserver?(.wireGuardAdapterLoggerContextPreparationStarted)
        let context = Unmanaged.passUnretained(self).toOpaque()
        initializationObserver?(.wireGuardAdapterLoggerContextPrepared)
        initializationObserver?(.wireGuardAdapterLoggerCallbackPreparationStarted)
        let callback = Self.goLoggerCallback
        initializationObserver?(.wireGuardAdapterLoggerCallbackPrepared)

        #if DEBUG
        if loggerInstallationPolicy == .diagnosticBypass {
            initializationObserver?(.wireGuardAdapterLoggerInstallationBypassed)
            initializationObserver?(.wireGuardAdapterSetupLogHandlerReturned)
            return
        }
        #endif

        initializationObserver?(.wireGuardAdapterWGSetLoggerCallStarted)
        wgSetLogger(context, callback)
        initializationObserver?(.wireGuardAdapterWGSetLoggerCallReturned)
    ...
    private func startWireGuardBackend(wgConfig: String) throws -> Int32 {
        ...
        initializationObserver?(.wireGuardAdapterWGTurnOnCallStarted)
        let handle = wgTurnOn(wgConfig, tunnelFileDescriptor)
        initializationObserver?(.wireGuardAdapterWGTurnOnCallReturned)
        if handle < 0 {
