...
# Audited WireGuard-only variant of the fork's bridge Makefile. The fork keeps
# api-apple.go and api-xray.go in the same Go package; naming api-apple.go on the
# build command is therefore required to keep LibXray out of libwg-go.a.
...
$(BUILDDIR)/libwg-go-$(1).a: $(GOROOT)/.prepared go.mod api-apple.go
	go build -ldflags=-w -trimpath -v -o "$(BUILDDIR)/libwg-go-$(1).a" -buildmode c-archive api-apple.go
